<x-layouts.super-admin title="Permission catalog">
        ...
        @if (! count($page['catalog']))
            <x-panel :padded="false">
                <x-empty-state icon="key"
                    ...
                    <x-slot:actions>
                        <x-button size="sm" :href="route('admin.roles')">Open the role editor</x-button>
                    </x-slot:actions>
                </x-empty-state>
            </x-panel>
        @else
                        ...
                        <ul class="divide-y divide-clay-100">
                            @foreach ($group['permissions'] as $permission)
                                <li class="px-4 py-2.5">
                                    ...
                                    <p class="mt-0.5 font-mono text-micro text-fern-400">{{ $permission['key'] }}</p>
